namespace TrafficManager.UI.SubTools.TimedTrafficLights {
          ...
          UI.MainMenu.IOnscreenDisplayProvider
    {
        ...
        private void GuiTimedControlPanelRefactored(int num) {
            TrafficLightSimulationManager tlsMan = TrafficLightSimulationManager.Instance;
        ...
        private void GuiTimedControlPanel(int num) {
            ...
            try {
                ...
                void BuildCurrentStepBarLightActive(int i, ITimedTrafficLights timedNodeMain) {
                    // current step
                    GUILayout.BeginVertical();
                    GUILayout.Space(5);
                ...
                void BuildOtherStepBarsLightActive(int i, ITimedTrafficLights timedNodeMain) {
                    GUILayout.Label(
                                    string.Format(
                                        T("Format:State {0}: {1}..{2}"),
                                        i + 1,
                ...
                void BuildBarInEditMode(int i, ITimedTrafficLights timedNodeMain) {
                    GUIStyle labelLayout = layout;
                ...
                void BuildBarToEditStep(int i,ITimedTrafficLights timedNodeMain) {
                    nodeSelectionLocked = true;
                    int oldStepMinValue = _stepMinValue;
                    int oldStepMaxValue = _stepMaxValue;
                ...
                void BuildBarAddStep(ITimedTrafficLights timedNodeMain) {
                    nodeSelectionLocked = true;
                ...
                // Check for the current state of the light
                if (Event.current.type == EventType.Layout) {
                    timedLightActive = tlsMan.HasActiveTimedSimulation(selectedNodeIds[0]);
                    currentStep = timedNodeMain.CurrentStep;
                    inTestMode = timedNodeMain.IsInTestMode();
                ...
                // Update the "real-world" traffic lights to the current current step.
                // Don't know if it affects the GUI or the simulation? Which one is the real world?
                if (!timedLightActive && numSteps > 0 && !_timedPanelAdd && _timedEditStep < 0 &&
                    _timedViewedStep < 0) {
                    _timedViewedStep = 0;
                    foreach (ushort nodeId in selectedNodeIds) {
                ...
                for (var i = 0; i < timedNodeMain.NumSteps(); i++) {
                    ...
                    // _timedEditStep is the step we are currently editing. It's -1 if we're not editing a step
                    // all of this interferes with the internal structure of the traffic light.
                    // In consequence, some of the functionalities should be encapsulated into the light itself.
                    //  If (i-th step is not in edit mode)
                    //      If (light is actice)
                    //         If (i-th is current step)
                    if (_timedEditStep != i) {
                        if (timedLightActive) {
                            if (i == currentStep) {
                                BuildCurrentStepBarLightActive(i, timedNodeMain);
                            } else {
                                // active light, not the current step
                                BuildOtherStepBarsLightActive(i, timedNodeMain);
                            }
                        } else {
                            // Inactive light
                            BuildBarInEditMode(i, timedNodeMain);
                        }
                    } else {
                        BuildBarToEditStep(i, timedNodeMain);
                    }
        ...
        private void GuiTimedTrafficLights() {
            ...
            _windowRect = GUILayout.Window(
                ...
                func: GuiTimedControlPanelRefactored,
